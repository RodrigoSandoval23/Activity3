...
/* ==================================================================
   AUTOMATED TERMINAL LOGGER (Middleware)
   This function intercepts every request to print CRUD actions.
   ================================================================== */
app.use((req, res, next) => {
  // 1. Map HTTP verbs to CRUD terminology
  const methodToCrud = {
    GET: "READ  ", // Spaces added for alignment in terminal
    POST: "CREATE",
    PUT: "UPDATE",
    DELETE: "DELETE",
  };

  const action = methodToCrud[req.method] || req.method;
  const timestamp = new Date().toLocaleTimeString();

  // 2. Print the Log
  console.log(`[${timestamp}] ${action} > ${req.url}`);

  // 3. Pass control to the next function (the actual routes)
  next();
});
/* ================================================================== */
